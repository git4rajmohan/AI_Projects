# How to Test -Enterprise Knowledge Assistant
...
> Automated suite: `pytest tests/` (30 tests) - see Section 6. This file covers the **manual/E2E** path.
...
|---|---------|---------|-----------------|
| 1 | **Local Ollama daemon** (`localhost:11434`) | Embeddings (`nomic-embed-text`) -cloud plan has NO embedding models | Usually auto-starts (Windows service / `ollama serve`) |
| 2 | **Qdrant** (`localhost:6333`) | Vector store (all 6 collections: chunks, entities, edges...) | **Yes - always** |
| 3 | **Ollama Cloud** (`ollama.com`) | LLM `gpt-oss:120b` | No -remote API; only needs `OLLAMA_API_KEY` in `.env` |
| 4 | **Streamlit app** (`localhost:8501`) | The UI under test | **Yes - always, after 1+2 are up** |

Startup order matters: **Ollama ->Qdrant ->(verify) ->Streamlit**.
The app reads Qdrant/Cognee state lazily, so both must be up before you chat.

---

## 2. How to start -exact commands (PowerShell)
...
ollama list
```
- If a model list prints ->daemon is up. If it errors ->start it:
```powershell
ollama serve
```
...
### 2b. Qdrant
️Must be started **with CWD = the `qdrant` folder**, or its storage lands in the wrong place:
```powershell
...
### 2c. Preflight -confirm ALL services before starting the app
```powershell
.\.venv\Scripts\python.exe scripts\check_services.py
```
Expected output -**4 green lines** (values real, never fabricated):
```
PASS:Local Ollama daemon: daemon up at http://localhost:11434, 27 models listed
PASS:Embedding model 'nomic-embed-text': 'nomic-embed-text' available on local daemon
PASS:Ollama Cloud 'gpt-oss:120b': 'gpt-oss:120b' responded (2 chars) via https://ollama.com/api
PASS:Qdrant: up at http://localhost:6333, version 1.19.1
PASS:ALL CHECKS PASSED
```
FAIL:Any red line ->fix that service first (see Section 8 Troubleshooting).
...
1. **Sidebar ->Documents**: lists **Indexed documents (11)** -all corpus PDFs
...
2. **Sidebar ->System Status**: click **Re-check services** **once** and WAIT -the spinner runs
   ~10-0 s (the Ollama Cloud ping is slow). Do **not** click repeatedly; each click restarts the
   ...
   | Cognee | Ready -1604 nodes / 4605 relationships |
   | Knowledge base | Indexed -131 chunks |
   *(A transient Cognee "Could not set lock ... Error 33" means two browser tabs read the graph
   simultaneously -re-check once more and it clears.)*
3. **Main area**: title + chat input box only (all controls moved to the sidebar).
4. **Sidebar**: Documents + System Status + Configuration + **️Retrieval options**
...
### TC-01 -Grounded answer with real citations (CORE)
**Steps:**
1. In the chat box type: `How many annual leave days do employees receive?`
2. Send (Enter).
3. Wait ~10-0 s (hybrid retrieval + cloud LLM generation).
4. Expand the **Sources** expander under the answer.

**Expected:**
- Answer describes tiered annual vacation days by tenure (observed: 10 days years 1-,
  15 days years 6-0, 20 days year 11+) -wording may vary, numbers must come from policy text.
- Answer contains inline evidence refs like `[1]`...`[n]`.
- **Sources** lists REAL corpus filenames, e.g. `pto-and-leave-policy.pdf` and/or
  `public_counsel_employee_handbook.pdf` -never invented names, never page numbers.
- Fail if: answer fabricates numbers not in policy, or sources are missing/unknown files.

### TC-02 -Unknown question ->no fabrication (CORE)
**Steps:**
...
- FAIL:Fail if the LLM invents a jet policy.

### TC-03 -Hybrid retrieval trace (debug mode)
**Steps:**
1. Toggle **Developer / debug mode** ON (**sidebar**, bottom).
2. Re-ask: `How are remote work and information security related?`
3. Expand ** Debug -retrieval trace**.
...
### TC-04 -Relationship question (graph leg)
**Steps:**
...
### TC-05 -Retrieval strategy toggle
**Steps:**
1. Sidebar -> **️Retrieval options**, set strategy to `vector`, re-ask TC-01's question.
...
### TC-06 -Example questions shortcut
**Steps:**
...
### TC-07 -CLI end-to-end (persistence, no re-ingestion)
...
- Starts fast (existing KB reused -proves nothing rebuilds on a fresh process).
- App terminal/log shows "11/11 skipped" style ledger behavior if ingestion were triggered -
  it should NOT trigger any ingestion at all.

### TC-07b -Knowledge-graph visualization (read-only, optional)
**Steps:**
1. Sidebar -> ** Knowledge graph view** -read the **What is this? / How to read it**
   guidance (explains columns, relationships, importance, search, layout tabs).
2. Click **Open graph view** ->opens a new browser tab at
   `http://localhost:8501/app/static/graph_visualization.html`.
3. Wait for the layout to finish ("Laying out graph..." -> **"Done!"**, up to ~2 min).
...
- Interactive page "Cognee Knowledge Graph": **500 nodes / 1,394 edges** in column layout
  (Documents ->Chunks ->Entities ->Types), node search, Story/Flow/Force layouts, dark mode,
  Schema/Memory/Semantic tabs; clicking a node shows its source chunk + provenance.
- CLI alternative (️stop Streamlit first -Ladybug lock):
  `.\.venv\Scripts\python.exe scripts\visualize_graph.py` ->writes
  `data/graph_visualization.html` (2.8 MB); add `--full` for the entire graph (1604 nodes).
- Fail if: the page never passes "Laying out graph..." (note: inside iframes the layout
  stalls at 0% -cognee's viz only works full-page, which is why the app opens a new tab),
  or "Could not set lock ... Error 33" (Streamlit still running during CLI export).

### TC-08 -Upload safety + processing (optional, writes to KB)

### TC-08 -Upload safety + processing (optional, writes to KB)
**Test data** -create `data/documents/travel-expense-note.txt` with exactly:
```
...
```
**Steps:**
1. Sidebar ->Upload documents ->select the .txt file.
2. Confirm it appears in the Documents list (sidebar auto-refreshes).
3. Click **Process documents** and wait (one small file 1-2 min via cloud LLM).
...
- Clicking **Process documents** again ->"Nothing to process -N file(s) already indexed"
  (hash-ledger duplicate detection).

### TC-09 -Unsafe filename rejection (optional, no KB writes)
**Test data:** a file named `bad name!.txt` (space + `!` -allowed chars pass) and one named
...
"Rejected unsafe filename" -never writes outside `data/documents/`.

### TC-10 -Clear knowledge base (️DESTRUCTIVE -run LAST only if required)
**Steps:**
1. Stop any other test. Note: re-ingesting the full corpus afterwards takes **~20-6 min**.
2. Sidebar -> **Clear knowledge base** popover -> **Yes, clear everything**.
3. Verify: Documents list empties metadata, status shows Cognee "not ingested yet", 0 Qdrant
   collections.
4. Rebuild: `.\.venv\Scripts\python.exe scripts\ingest_corpus.py` (~20-6 min; 11/11 processed
...
Corpus: 11 PDFs in `data/documents/`. Known-answer facts (verified against the actual PDF text -
use these to judge groundedness):

| Question topic | Document(s) | Ground-truth fact |
|---|---|---|
| Annual/vacation leave | `pto-and-leave-policy.pdf`, `public_counsel_employee_handbook.pdf` | Tiered by tenure -10 days (early years), 15 (mid), 20 (11+ yrs); sick leave separate (10 days/80 hrs) |
...
2026 picnic schedule, quantum-computing policy -none of these exist in the corpus.
...
Extraction is LLM-driven, so re-ingested counts may differ slightly -that is expected;
...
️**Stop Streamlit first** (Ctrl+C) -the Ladybug graph DB file-locks and a second process's
pytest dies with "Could not set lock (.lbug, Error 33)".

```powershell
.\.venv\Scripts\python.exe -m pytest tests\ -v
```
Expected: **82 passed** in ~7- min (config 9, groundedness 4, health 3, ingestion 4, qdrant 3,
retrieval 9, evidence-gate 9, bm25 7, fusion 7, golden-dataset 9, evaluation 18, plus the
Phase 1-3 suites). The cloud-LLM leg auto-skips (never fails) if the paid endpoint is
...
- [ ] `check_services.py` ->4/4 green
- [ ] UI ready state: 11 docs + 6 green status lines + chat box
- [ ] TC-01 grounded answer + real sources
- [ ] TC-02 unknown question ->not-found, no fabrication
- [ ] TC-03 debug trace shows both vector + graph evidence
- [ ] TC-07 CLI answers from existing KB with zero re-ingestion
- [ ] (if run) TC-08 upload ->process ->cited answer ->duplicate skip
- [ ] `pytest tests\` ->82 passed

## 7b. Production-grade layer checks (enforcement gate + evaluation)

**Gate (manual, in the app or CLI):** ask *ho won the 1998 FIFA World Cup final?- ->
the answer must be the standard refusal - don't have enough evidence...-and the debug
panel must show **Evidence gate: REFUSED** with The LLM was not called. A real policy
...
| Symptom | Cause / Fix |
|---|---|
| `check_services` Qdrant FAIL:| qdrant.exe not started from the `qdrant` folder -restart per Section 2b |
| Status lines blank / spinner forever | One check cycle takes 10-0 s; click Re-check **once** and wait. Repeated clicks restart the checks |
| Cognee "Could not set lock ... Error 33" | Two tabs/processes reading the graph at once -close one, re-check; transient |
| sqlite3 "unable to open database file" | Cognee import-order bug -report it, don't chase DB locks |
| "No documents indexed yet" but KB has data | Regenerate metadata: `DocumentManager(...).sync()` -the sidebar now self-heals this on render |
| KB status "not ingested yet" / dataset not found | Re-ingest: `scripts\ingest_corpus.py` (~20-6 min; ledger skips unchanged files) |
| Chat answer has no sources | Check debug trace: if vector hits are empty, Qdrant collection may be empty ->re-ingest |
| `visualize_graph.py` hangs after "HTML written" | Known cosmetic hang in cognee 1.4.2 post-export cleanup -if `data/graph_visualization.html` exists and ends with `</html>`, the export succeeded; Ctrl+C the script and open the file directly |
| Full wipe wanted | `scripts\reset_database.py --yes` (keeps the 11 PDFs) -stop Streamlit first |
