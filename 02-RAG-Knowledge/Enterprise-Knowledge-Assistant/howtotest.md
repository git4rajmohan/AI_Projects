...
> Automated suite: `pytest tests/` (30 tests) — see §6. This file covers the **manual/E2E** path.
...
### 2b. Qdrant
⚠️ Must be started **with CWD = the `qdrant` folder**, or its storage lands in the wrong place:
```powershell
...
✅ Embedding model 'nomic-embed-text': 'nomic-embed-text' available on local daemon
✅ Ollama Cloud 'gpt-oss:120b': 'gpt-oss:120b' responded (2 chars) via https://ollama.com/api
✅ Qdrant: up at http://localhost:6333, version 1.19.1
✅ ALL CHECKS PASSED
```
❌ Any red line → fix that service first (see §8 Troubleshooting).
...
2. **Sidebar → System Status**: click **Re-check services** **once** and WAIT — the spinner runs
   ~10–30 s (the Ollama Cloud ping is slow). Do **not** click repeatedly; each click restarts the
...
4. **Sidebar**: Documents + System Status + Configuration + **⚙️ Retrieval options**
...
2. Send (Enter).
3. Wait ~10–40 s (hybrid retrieval + cloud LLM generation).
4. Expand the **Sources** expander under the answer.

**Expected:**
- Answer describes tiered annual vacation days by tenure (observed: 10 days years 1–5,
  15 days years 6–10, 20 days year 11+) — wording may vary, numbers must come from policy text.
...
- ❌ Fail if the LLM invents a jet policy.
...
**Steps:**
1. Sidebar → **⚙️ Retrieval options**, set strategy to `vector`, re-ask TC-01's question.
...
2. Click **🕸️ Open graph view** → opens a new browser tab at
...
- CLI alternative (⚠️ stop Streamlit first — Ladybug lock):
...
3. Click **Process documents** and wait (one small file ≈ 1–3 min via cloud LLM).
...
### TC-10 — Clear knowledge base (⚠️ DESTRUCTIVE — run LAST only if required)
**Steps:**
1. Stop any other test. Note: re-ingesting the full corpus afterwards takes **~20–26 min**.
...
3. Verify: Documents list empties metadata, status shows Cognee "not ingested yet", 0 Qdrant
   collections.
4. Rebuild: `.\.venv\Scripts\python.exe scripts\ingest_corpus.py` (~20–26 min; 11/11 processed
...
```
Expected: **82 passed** in ~7–8 min (config 9, groundedness 4, health 3, ingestion 4, qdrant 3,
retrieval 9, evidence-gate 9, bm25 7, fusion 7, golden-dataset 9, evaluation 18, plus the
Phase 1–13 suites). The cloud-LLM leg auto-skips (never fails) if the paid endpoint is
...
**Gate (manual, in the app or CLI):** ask *“Who won the 1998 FIFA World Cup final?”* →
the answer must be the standard refusal “I don't have enough evidence…” and the debug
...
| Symptom | Cause / Fix |
|---|---|
| `check_services` Qdrant ❌ | qdrant.exe not started from the `qdrant` folder — restart per §2b |
| Status lines blank / spinner forever | One check cycle takes 10–30 s; click Re-check **once** and wait. Repeated clicks restart the checks |
...
| KB status "not ingested yet" / dataset not found | Re-ingest: `scripts\ingest_corpus.py` (~20–26 min; ledger skips unchanged files) |
